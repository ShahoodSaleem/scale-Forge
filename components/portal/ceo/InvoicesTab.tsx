...
import {
  ...
  ArrowLeft, Clock, CheckCircle2, AlertCircle, XCircle, FileMinus
} from "lucide-react";
...
export default function CeoInvoicesTab({ addToast }: { addToast: (type: "success" | "error" | "info", msg: string) => void }) {
  ...
  const handleCreate = async () => {
    ...
    setShowAdd(false);
    setForm({ client_name: "", client_email: "", issued_date: new Date().toISOString().split("T")[0], due_date: "", tax_rate: "0", notes: "", status: "unpaid" });
        ...
        <div className="rounded-2xl border border-white/10 bg-white/3 p-8 space-y-8">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <p className="text-3xl font-bold text-white">{fmtMoney(detail.total)}</p>
            </div>
          </div>
            ...
            <div className="rounded-xl border border-white/8 overflow-hidden">
              ...
              {invItems.length === 0 ? (
                ...
                <div key={item.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-3 border-t border-white/5 text-sm">
                  ...
                  <span className="text-white/40">{fmtMoney(item.unit_price)}</span>
                  <span className="text-white text-right font-semibold">{fmtMoney(item.total)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-2 pt-4 border-t border-white/8">
            <div className="flex gap-8 text-sm"><span className="text-white/40">Subtotal</span><span className="text-white font-medium">{fmtMoney(detail.subtotal)}</span></div>
            <div className="flex gap-8 text-sm"><span className="text-white/40">Tax ({detail.tax_rate}%)</span><span className="text-white font-medium">{fmtMoney(detail.tax_amount)}</span></div>
            <div className="flex gap-8 text-base font-bold border-t border-white/10 pt-2 mt-1"><span className="text-white/60">Total</span><span className="text-white">{fmtMoney(detail.total)}</span></div>
          </div>
          {detail.paid_at && (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/3 p-5">
          ...
          <p className="text-2xl font-bold text-white">{fmtMoney(invoices.reduce((s, i) => s + i.total, 0))}</p>
        ...
        {filtered.length === 0 ? (
          ...
          <motion.div key={inv.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <div className="flex items-center gap-6">
              ...
              <p className="text-white font-bold text-sm min-w-[80px] text-right">{fmtMoney(inv.total)}</p>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {lineItems.map((li, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <span className="text-white/40 text-xs w-16 text-right">{fmtMoney(calcLineTotal(li))}</span>
                        {lineItems.length > 1 && (
                  ...
                  <div className="flex justify-end mt-3 text-sm gap-3">
                    ...
                    <span className="text-white font-bold">{fmtMoney(calcSubtotal())}</span>
                  </div>
                </div>
                <div>
